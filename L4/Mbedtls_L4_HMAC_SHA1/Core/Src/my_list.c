...
node* CreatList(void *content) {
	node *listHead = my_malloc(LIST_SRAM, sizeof(node));
	if (listHead != NULL){
...
node* InsertList(void *content, node *list, node *position) {
    ...
    insertNode = my_malloc(LIST_SRAM, sizeof(struct node));
...
node *DeleteNode(void *content, node *list) {
    ...
    if (isLast(prevPosition)) {
       return NULL;
        ...
        my_free(LIST_SRAM, deleteNode->content);
		my_free(LIST_SRAM, deleteNode);
		return list;
    }
}
