...
unsigned int UART_StaInit(UART_Sta *uartSta, UART_HandleTypeDef *uart, DMA_HandleTypeDef *dma_rx) {
	...
	uartSta->recvCplt = 0;
	uartSta->RxLen = 0;
	uartSta->RxBuf = my_malloc(SRAM12, RX_BUF_SIZE);
	if (uartSta->RxBuf) {
...
/*
**********************************************************************************
* Name: UART_SendData(UART_HandleTypeDef uart, char *buf)	
* Description:
* Parameter(s):
*	UART_HandleTypeDef uart
*	char *buf
...
char* UART_ContGetData(UART_Sta *uartSta, unsigned int continuous_rev_time) {
	...
	while (continuous_rev_time > (HAL_GetTick() - continuous_rev_tick_start)) {
		if (uartSta->recvCplt == 1) {
			...
			// 为新数据和之前的数据重新分配空间
			if (uartSta->RxLen != 0) {
				data = my_realloc(SRAM12, data, rev_data_len + uartSta->RxLen);
				// 如果空间成功分配
				if (data != NULL) {
					// 将buf里的数据拷贝到原有数据后面
...
char* UART_WaitOnRevData(UART_Sta *uartSta, unsigned int timeout) {
	...
	uartSta->RxLen = my_strlen((char *)uartSta->RxBuf);
	data = my_malloc(SRAM12, uartSta->RxLen);
	my_memclear(data, uartSta->RxLen + 1);
	if (data != NULL) {
